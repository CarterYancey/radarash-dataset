...
The pipeline is complete: `make all` produces `dataset_v1.2` from a raw
...
registry), computes the registry-driven ranks/sector-ranks under each
...
immutable `data/datasets/dataset_v1.2/` — `dataset.parquet` + the split
...
From an existing ingest, `make all` runs steps 2–6 end-to-end.
